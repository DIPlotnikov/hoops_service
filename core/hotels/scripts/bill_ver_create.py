...
class BillCreatorGCD(object):
        ...
        *,
        ...
        <table style=\"width: 100%; border-collapse: collapse; page-break-inside: avoid; margin: 0; padding: 0; table-layout: fixed; font-size: 12px;\">
            <tr>
                ...
                <td style=\"width:12%; text-align: center; border: 1px solid #000;\"><div class=\"cell-content\">{3}</div></td>
                <td style=\"width:10%; text-align: center; border: 1px solid #000;\"><div class=\"cell-content\">{4}</div></td>
                <td style=\"width:10%; text-align: center; border: 1px solid #000;\"><div class=\"cell-content\">{2}</div></td>
            </tr>
        </table>"""
    ...
    def createBill(
        self,
        path_from: str = f"./{core}/scripts/sample/bill_GCD.html",
        path_to: str = f"/usr/local/share/minio/{bucket}/bills",
        object: str = "hotel",
        verification: bool = True,
        nds: float = 0.0,
        ...
        if goods is None:
            goods = [("Подтверждение реквизитов организации и акцепт договора-оферты", 1.00, 1, 1)]
        if verification:
            self._file_path = os.path.join(path_to, object, self._number, "bill_GCD.html")
        else:
            current_datetime = self.closing_date
            suffix = "hoops" if is_hoops else "executer"
            self._file_path = os.path.join(
    ...
    def send_bill_for_verification(self, url, nds):
        ...
        """
        path_from = f"./{core}/scripts/sample/bill_GCD.html"
        goods = ("Подтверждение реквизитов организации и акцепт договора-оферты", 1.00, 1, 1)
